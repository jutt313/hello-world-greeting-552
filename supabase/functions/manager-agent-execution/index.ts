...
// Complete Manager Agent System Prompt
...
interface ManagerRequest {
  action: 'chat' | 'plan_project' | 'execute_task' | 'coordinate_agents';
  user_id: string;
  project_id: string;
  message?: string;
  context?: any;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, user_id, project_id, message, context }: ManagerRequest = await req.json()

    console.log(`Manager Agent - Action: ${action}, User: ${user_id}, Project: ${project_id}`)
    ...
    // Get project context and memory for intelligent decision making
    ...
    const { data: chatHistory } = await supabaseClient
      .from('chat_messages')
      .select('*')
      .eq('session_id', project_id) // Using project_id as session for now
    ...
    if (message) {
      ...
      await supabaseClient
        ...
        .insert({
          ...
          content: response,
          sender_type: 'agent',
          sender_agent_id: 'manager_supreme',
    ...
    // Store important insights as memories for future reference
    if (response.length > 500) { // Only store substantial responses
      await supabaseClient
        .from('agent_memory_contexts')
        .insert({
          agent_id: 'manager_supreme',
          project_id: project_id,
          memory_type: 'conversation',
          context_key: `user_interaction_${Date.now()}`,
          context_data: {
            user_message: message,
            manager_response: response.substring(0, 1000), // Store first 1000 chars
            timestamp: new Date().toISOString(),
            action_type: action
          },
          relevance_score: 0.8,
          tags: ['user_interaction', action]
        })
    }
