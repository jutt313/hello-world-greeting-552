...
const LLMProviderAnalytics: React.FC<LLMProviderAnalyticsProps> = ({ provider, isOpen, onClose }) => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const totalCalls = data.reduce((sum, record) => sum + (record.api_calls || 0), 0);
      ...
      data.forEach(record => {
        ...
        if (!dailyMap.has(date)) {
          dailyMap.set(date, { date, tokens: 0, calls: 0, cost: 0 });
        }
        const day = dailyMap.get(date);
        day.tokens += record.tokens_used || 0;
        day.calls += record.api_calls || 0;
      ...
      data.forEach(record => {
        if (!modelMap.has(record.model_name)) {
          modelMap.set(record.model_name, { model: record.model_name, usage: 0, color: colors[modelMap.size % colors.length] });
        }
    ...
    if (isOpen) {
      ...
      const channel = supabase.channel('analytics-updates')
        .on('postgres_changes', {
          ...
          filter: `provider_name=eq.${provider.provider_name}`
        }, fetchAnalytics)
        .subscribe();

      return () => supabase.removeChannel(channel);
    }
  }, [isOpen, provider.provider_name]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl rounded-xl max-h-[90vh] overflow-y-auto">
        ...
        {!analytics || (analytics.totalCalls === 0 && analytics.totalTokens === 0) ? (
