...
import { User, Settings, FileText, Bot, LogOut, ChevronDown } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { useNavigate } from 'react-router-dom';
import { ManagerAgentPopup } from '@/components/popups/ManagerAgentPopup';
...
const ProfileDropdown = () => {
  ...
  const menuItems = [
    { 
      icon: Bot, 
      label: 'Chat with Platform Agent', 
      action: () => setIsManagerChatOpen(true) 
    },
  ...
  return (
    <>
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute right-0 top-full mt-2 w-56 bg-card border border-border rounded-lg shadow-lg z-[9999]">
      ...
      <ManagerAgentPopup
        isOpen={isManagerChatOpen}
        onClose={() => setIsManagerChatOpen(false)}
      />
    </>
  );
};
