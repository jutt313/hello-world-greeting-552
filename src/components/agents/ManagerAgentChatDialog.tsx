...
import { Crown, Send, Upload, GitBranch, User, Bot, Loader2, FolderOpen } from 'lucide-react';
    ...
    try {
      ...
      const response = await fetch('https://akoclehzeocqlgmmbkza.supabase.co/functions/v1/manager-agent-execution', {
        ...
        body: JSON.stringify({
          ...
          has_project_context: uploadedFiles.length > 0 || gitRepoUrl !== ''
        }),
      });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          {(uploadedFiles.length > 0 || currentProjectId) && (
            <div className="bg-muted p-3 rounded-lg">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <FolderOpen className="w-4 h-4" />
                <span>
                  Project Context: {uploadedFiles.length > 0 ? `${uploadedFiles.length} files uploaded` : 'Chat session project'}
                </span>
              </div>
            </div>
          )}
